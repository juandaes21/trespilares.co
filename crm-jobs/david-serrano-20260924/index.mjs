...
try {
  ...
  if (!ownerRes.rowCount) throw new Error("No active CRM user matching Juan David");
  ...
  const activityRes = await db.query(
    `SELECT id FROM crm_activities
      WHERE contact_id=$1 AND type='linkedin_connection' AND summary=$2
  ...
  if (activityRes.rowCount) {
    ...
    await db.query(
      `INSERT INTO crm_activities
       ...
       VALUES ($1,$2,'linkedin_connection','inbound',$3,NOW(),$4)`,
  ...
  const taskRes = await db.query(
    `SELECT id FROM crm_tasks
      WHERE contact_id=$1 AND type='linkedin_dm' AND status='open' AND title=$2
  ...
  if (taskRes.rowCount) {
    ...
    await db.query(
      `INSERT INTO crm_tasks
       ...
       VALUES ($1,$2,'linkedin_dm',$3,NOW(),'open','normal',$4,$4)`,
  ...
  const verifyRes = await db.query(
    `SELECT c.id,c.name,c.stage,c.source_channel,c.source_profile,u.name AS owner_name,
      EXISTS(SELECT 1 FROM crm_activities a WHERE a.contact_id=c.id AND a.type='linkedin_connection') AS activity_ok,
      EXISTS(SELECT 1 FROM crm_tasks t WHERE t.contact_id=c.id AND t.type='linkedin_dm' AND t.status='open') AS task_ok
      FROM crm_contacts c
